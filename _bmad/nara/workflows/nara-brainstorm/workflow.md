...
description: Phase 1 — Brainstorming & discovery for a new Nara feature. Runs ideation techniques, optional research, and produces brainstorm.md before the PRD. Requires context.md from /nara-start.
...
### Step 1 — Load config & feature folder

Load `{project-root}/_bmad/bmm/config.yaml`. Resolve `output_folder`, `user_name`.

Resolve `{feature_folder}` from argument or ask user for the feature slug.
...
Load and follow `{project-root}/.claude/skills/bmad-brainstorming/workflow.md` with the following Nara-specific context injected as the brainstorming topic:
...
On **[D]** → Load and follow `{project-root}/_bmad/bmm/workflows/1-analysis/research/workflow-domain-research.md`

On **[T]** → Load and follow `{project-root}/_bmad/bmm/workflows/1-analysis/research/workflow-technical-research.md`
...
Output:
```
✅ Phase 0 complete — Brainstorm session done.
